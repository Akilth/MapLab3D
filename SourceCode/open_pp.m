...
try
	...
	if ~isempty(ELE)
		if size(PP_temp.colorspec,1)>size(ELE.ifs_v,1)
			errormessage(sprintf([...
				'The number of color specs in the project parameters (%g) is greater\n',...
				'than the number of color specs when loading the OSM data (%g).\n',...
				'The OSM data must be reloaded.\n',...
				'Loading the project parameters has been canceled.'],...
				size(PP_temp.colorspec,1),...
				size(ELE.ifs_v,1)));
		end
	end
	...
	% If the elevation data has been loaded:
	% When loading the project parameters, the color numbers of the objects could have been changed.
	% In this case, also the field ELE.elecolor has to be updated:
	if ~isempty(ELE)
	...
	for i=1:size(pp_used,1)
		if pp_used(i,1).colno>0
			while colno<=pp_used(i,1).colno
				text_str	= sprintf('%s%s\t%s\t\t%s\t%s\t%s: %s %s %s\t\t%s\n',text_str,...
