...
try
	...
	for iseqt=1:size(PLOTDATA.obj(iobj,1).symb,1)
		...
		% Source:
		% Create only one source plot for each index iseqt and isource to reduce the amount of data!
		...
		for isource=1:size(PLOTDATA.obj(iobj,1).symb(iseqt,1).source,1)
			allsourceplots(isource,1).source		= [];
			ud_source				= [];
			...
			if ~isempty(PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1))
				% Source plot of the nodes:
				if ~isempty(PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.nodes)
					if ~ishandle(GV_H.ax_2dmap)
						errormessage(sprintf(['There exists no map where to plot the objects.\n',...
							'Create the map first.']));
					end
					allsourceplots(isource,1).source(end+1,1).h	= plot(GV_H.ax_2dmap,...
						PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.nodes.xy(:,1),...
						PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.nodes.xy(:,2),...
				...
				% Source plot of lines and areas:
				source_liar_xy		= zeros(0,2);
				if ~isempty(PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.lines)
					for k=1:size(PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.lines,1)
						if isempty(source_liar_xy)
							source_liar_xy		= [...
								PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.lines(k,1).xy(:,1) ...
								PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.lines(k,1).xy(:,2)];
						else
							source_liar_xy		= [source_liar_xy;nan nan;...
								PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.lines(k,1).xy(:,1) ...
								PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.lines(k,1).xy(:,2)];
						end
					end
				end
				if ~isempty(PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.areas)
					for k=1:size(PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.areas,1)
						if isempty(source_liar_xy)
							source_liar_xy		= [...
								PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.areas(k,1).xy(:,1) ...
								PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.areas(k,1).xy(:,2)];
						else
							source_liar_xy		= [source_liar_xy;nan nan;...
								PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.areas(k,1).xy(:,1) ...
								PLOTDATA.obj(iobj,1).symb(iseqt,1).source(isource,1).connways.areas(k,1).xy(:,2)];
				...
				if ~isempty(source_liar_xy)
					if isnan(source_liar_xy(1,1))||isnan(source_liar_xy(1,2))
						errormessage;
					end
					if ~ishandle(GV_H.ax_2dmap)
						errormessage(sprintf(['There exists no map where to plot the objects.\n',...
				...
				for ipoly=1:size(PLOTDATA.obj(iobj,1).symb(iseqt,1).poly_symb_obj,1)
					...
					if ~isempty(PLOTDATA.obj(iobj,1).symb(iseqt,1).poly_symb_bgd)
						if (sum(numboundaries(PLOTDATA.obj(iobj,1).symb(iseqt,1).poly_symb_bgd(ipoly,1)))>0) && ...
							...
							% UserData:
							ud					= PLOTDATA.obj(iobj,1).symb(iseqt,1).ud_symb_bgd(ipoly,1);
							% shape0:
							ud.shape0		= PLOTDATA.obj(iobj,1).symb(iseqt,1).poly_symb_bgd(ipoly,1);
							% Source handle:
							isource			= PLOTDATA.obj(iobj,1).symb(iseqt,1).isource(ipoly,1);
							ud.source		= allsourceplots(isource,1).source;
							% plot-command:
							if isequal(ud.color_no,0)
								facecolor	= 'none';
								linewidth	= GV.colorno_e0_linewidth;
							else
								facecolor	= PP.color(ud.color_no).rgb/255;
					...
					% Symbol foreground:
					% UserData:
					ud					= PLOTDATA.obj(iobj,1).symb(iseqt,1).ud_symb_obj(ipoly,1);
					% shape0:
					ud.shape0		= PLOTDATA.obj(iobj,1).symb(iseqt,1).poly_symb_obj(ipoly,1);
					% Source handle:
					isource			= PLOTDATA.obj(iobj,1).symb(iseqt,1).isource(ipoly,1);
					ud.source		= allsourceplots(isource,1).source;
					% plot-command:
	...
	for iteqt=1:size(PLOTDATA.obj(iobj,1).text,1)
		...
		% The source plots are made visible, if the corresponding text or symbol is selected.
		% This makes it easier to move the texts and symbols to the right place when editing the map.
		for isource=1:size(PLOTDATA.obj(iobj,1).text(iteqt,1).source,1)
			allsourceplots(isource,1).source		= [];
			ud_source				= [];
			ud_source.issource	= true;	% to recognize it as source
			ud_source.imapobj		= 0;		% save_project: save the index imapobj
			...
			if ~isempty(PLOTDATA.obj(iobj,1).text(iteqt,1).source)
				% Source plot of the nodes:
				if ~isempty(PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.nodes)
					if ~ishandle(GV_H.ax_2dmap)
						errormessage(sprintf(['There exists no map where to plot the objects.\n',...
							'Create the map first.']));
					end
					allsourceplots(isource,1).source(end+1,1).h	= plot(GV_H.ax_2dmap,...
						PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.nodes.xy(:,1),...
						PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.nodes.xy(:,2),...
						'Color'     ,GV.tempprev.Color,...
						'LineStyle' ,'none',...
						'Marker'    ,GV.tempprev.Marker,...
						'MarkerSize',GV.tempprev.MarkerSize,...
				...
				% Source plot of lines and areas:
				source_liar_xy		= zeros(0,2);
				if ~isempty(PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.lines)
					for k=1:size(PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.lines,1)
						if isempty(source_liar_xy)
							source_liar_xy		= [...
								PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.lines(k,1).xy(:,1) ...
								PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.lines(k,1).xy(:,2)];
						else
							source_liar_xy		= [source_liar_xy;nan nan;...
								PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.lines(k,1).xy(:,1) ...
								PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.lines(k,1).xy(:,2)];
				...
				if ~isempty(PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.areas)
					for k=1:size(PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.areas,1)
						if isempty(source_liar_xy)
							source_liar_xy		= [...
								PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.areas(k,1).xy(:,1) ...
								PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.areas(k,1).xy(:,2)];
						else
							source_liar_xy		= [source_liar_xy;nan nan;...
								PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.areas(k,1).xy(:,1) ...
								PLOTDATA.obj(iobj,1).text(iteqt,1).source(isource,1).connways.areas(k,1).xy(:,2)];
				...
				for ipoly=1:size(PLOTDATA.obj(iobj,1).text(iteqt,1).poly_text_obj,1)
					...
					if ~isempty(PLOTDATA.obj(iobj,1).text(iteqt,1).poly_text_bgd)
						if (sum(numboundaries(PLOTDATA.obj(iobj,1).text(iteqt,1).poly_text_bgd(ipoly,1)))>0) && ...
							...
							% UserData:
							ud					= PLOTDATA.obj(iobj,1).text(iteqt,1).ud_text_bgd(ipoly,1);
							% shape0:
							ud.shape0		= PLOTDATA.obj(iobj,1).text(iteqt,1).poly_text_bgd(ipoly,1);
							% Source handle:
							isource			= PLOTDATA.obj(iobj,1).text(iteqt,1).isource(ipoly,1);
							ud.source		= allsourceplots(isource,1).source;
							% plot-command:
							if isequal(ud.color_no,0)
								facecolor	= 'none';
					...
					% Text foreground:
					% UserData:
					ud					= PLOTDATA.obj(iobj,1).text(iteqt,1).ud_text_obj(ipoly,1);
					% shape0:
					ud.shape0		= ...
						PLOTDATA.obj(iobj,1).text(iteqt,1).poly_text_obj(ipoly,1);
					% Source handle:
					...
					if numboundaries(PLOTDATA.obj(iobj,1).text(iteqt,1).poly_text_lrp(ipoly,1))>0
						% UserData:
						ud					= PLOTDATA.obj(iobj,1).text(iteqt,1).ud_text_lrp(ipoly,1);
						% shape0:
						ud.shape0		= PLOTDATA.obj(iobj,1).text(iteqt,1).poly_text_lrp(ipoly,1);
						% Source data: not assigned to the connectine line.
						% plot-command:
