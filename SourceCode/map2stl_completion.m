...
try
	...
	if save_summary
		...
		% files.size_x(i_file,1)
		% files.size_y(i_file,1)
		% files.size_z(i_file,1)
		...
		if isfield(PRINTDATA,'frame')
			if    isfield(PRINTDATA.frame,'tile')&&...
				...
				for tile_no=1:size(PRINTDATA.frame.tile,1)
					...
					files.i_colprio_nonstal(i_file,1)	= 0;		% stand-alone color
					files.area_nonstal(i_file,1)			= 0;		% stand-alone color
					if    ~isempty(PRINTDATA.frame.tile(tile_no,1).xmin)&&...
							~isempty(PRINTDATA.frame.tile(tile_no,1).xmax)&&...
							~isempty(PRINTDATA.frame.tile(tile_no,1).ymin)&&...
							~isempty(PRINTDATA.frame.tile(tile_no,1).ymax)&&...
							~isempty(PRINTDATA.frame.tile(tile_no,1).zmin)&&...
							~isempty(PRINTDATA.frame.tile(tile_no,1).zmax)
						files.size_x(i_file,1)				= ...
						...
						files.size_x(i_file,1)				= 0;
						files.size_y(i_file,1)				= 0;
						files.size_z(i_file,1)				= 0;
					end
				end
			end
		end
										...
										if    isfield(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno),'i_part_stal_v')&&...
											...
											if    ~isempty(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).i_part_stal_v)&&...
												...
												if ~isempty(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).T_print.Points)
													files.size_x(i_file,1)				= ...
														max(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).T_print.Points(:,1))-...
														min(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).T_print.Points(:,1));
													files.size_y(i_file,1)				= ...
														max(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).T_print.Points(:,2))-...
														min(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).T_print.Points(:,2));
													files.size_z(i_file,1)				= ...
														max(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).T_print.Points(:,3))-...
														min(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).T_print.Points(:,3));
												else
													files.size_x(i_file,1)				= 0;
													files.size_y(i_file,1)				= 0;
													files.size_z(i_file,1)				= 0;
													...
													if    isfield(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).col(i_colprio),'area')        &&...
														...
														if 	~isequal(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).col(i_colprio).area,0)      &&...
															...
															if ~isempty(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).col(i_colprio).T_print.Points)
																files.size_x(i_file,1)				= ...
																	max(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).col(i_colprio).T_print.Points(:,1))-...
																	min(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).col(i_colprio).T_print.Points(:,1));
																files.size_y(i_file,1)				= ...
																	max(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).col(i_colprio).T_print.Points(:,2))-...
																	min(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).col(i_colprio).T_print.Points(:,2));
																files.size_z(i_file,1)				= ...
																	max(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).col(i_colprio).T_print.Points(:,3))-...
																	min(PRINTDATA.tile(i_tile).col_stal(i_colprio_stal).file(fileno).col(i_colprio).T_print.Points(:,3));
															else
																files.size_x(i_file,1)				= 0;
																files.size_y(i_file,1)				= 0;
																files.size_z(i_file,1)				= 0;
		...
		text_str	= sprintf('%s\t\tColor\tColor\tColor\tColor\tColor\t\t\tColor\tPieces\tArea\n',text_str);
		text_str	= sprintf('%s\t\tDescription\tManufacturer\tBrand\tShort text\tstand-alone\t\t\tnon stand-alone\t\tmm^2\n',text_str);
		no_parts_total	= 0;
		for i_colprio=1:size(colprio_visible_local,1)
			...
			text_str	= sprintf('%s\t\t%s\t%s\t%s\t%s\t%s\t\t\t%s\t%1.0f\t%s\n',...
				text_str,...
				PP_local.color(colno,1).description,...
				PP_local.color(colno,1).manufacturer,...
		...
		text_str					= sprintf('%sFilename\tPrint\tColor\tColor\tColor\tColor\tColor\tTile\tFile\tColor\tPieces\tArea\tSize x\tSize y\tSize z\n',text_str);
		text_str					= sprintf('%s\t\tDescription\tManufacturer\tBrand\tShort text\tstand-alone\t\t\tnon stand-alone\t\tmm^2\tmm\tmm\tmm\n',text_str);
		no_parts_total			= 0;
		...
		for k=1:size(i_file_sort_v,1)
			...
			% Add data to text_str:
			text_str	= sprintf('%s%s\t%s\t%s\t%s\t%s\t%s\t%s\t%1.0f\t%1.0f\t%s\t%1.0f\t%s\t%s\t%s\t%s\n',text_str,...
				files.filename_stl{i_file,1},...
				printno_str,...
				PP_local.color(colno_i_file,1).description,...
				PP_local.color(colno_i_file,1).manufacturer,...
				...
				number2str(ceil(files.size_x(i_file,1)*100)/100,'%1.2f'),...
				number2str(ceil(files.size_y(i_file,1)*100)/100,'%1.2f'),...
				number2str(ceil(files.size_z(i_file,1)*100)/100,'%1.2f'));
